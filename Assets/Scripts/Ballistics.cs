...
    /// Gets the ballistic path.
    ...
    {
        float time = (vel * Mathf.Sin(angle) + Mathf.Sqrt(Mathf.Pow(vel*Mathf.Sin(angle),2) + 2 * -Physics.gravity.y * height)) / -Physics.gravity.y;
        //return (2.0f * vel * Mathf.Sin(angle)) / -Physics.gravity.y;
        Debug.Log("time1: " + (2.0f * vel * Mathf.Sin(angle)) / -Physics.gravity.y);
